...
class DXInterpreter {
   ...
   *   @option {string}   renderMode       'embedded' (default) | 'modal' | 'fullscreen'
   *   @option {Object}   modalOptions     Modal configuration (size, backdrop, keyboard)
   *   @option {Function} onStepComplete   Callback after each step submission
   *   @option {Function} onModalClose     Callback when modal is closed
   */
  constructor(target, options = {}) {
    ...
    this._lastVisibilitySignature = '';
    
    // Modal support
    this._renderMode = options.renderMode ?? 'embedded';
  ...
  /**
   * Create and show a Bootstrap modal containing the DX form
   */
  _createModal() {
    const modalOpts = this._options.modalOptions ?? {};
    const size = modalOpts.size ?? 'lg';
    const backdrop = modalOpts.backdrop ?? true;
    const keyboard = modalOpts.keyboard ?? true;

    // Create modal structure
    const modalDiv = _el('div', 'modal fade');
    modalDiv.id = 'dx-modal-' + Math.random().toString(36).substr(2, 9);
    modalDiv.setAttribute('tabindex', '-1');
    modalDiv.setAttribute('aria-hidden', 'true');
    if (backdrop === 'static') modalDiv.setAttribute('data-bs-backdrop', 'static');
    if (!keyboard) modalDiv.setAttribute('data-bs-keyboard', 'false');

    const modalDialog = _el('div', `modal-dialog modal-dialog-centered modal-dialog-scrollable modal-${size}`);
    const modalContent = _el('div', 'modal-content');
    
    const modalHeader = _el('div', 'modal-header border-0 pb-0');
    const closeBtn = _el('button', 'btn-close');
    closeBtn.setAttribute('type', 'button');
    closeBtn.setAttribute('data-bs-dismiss', 'modal');
    closeBtn.setAttribute('aria-label', 'Close');
    modalHeader.appendChild(closeBtn);

    const modalBody = _el('div', 'modal-body pt-0');
    modalBody.classList.add('dx-root'); // Add dx-root for styling
    ...
    // Store references
    this._modalElement = modalDiv;
    this._modalBody = modalBody;

    // Initialize Bootstrap modal
    if (typeof bootstrap !== 'undefined' && bootstrap.Modal) {
      this._modalInstance = new bootstrap.Modal(modalDiv, {
        backdrop: backdrop,
        keyboard: keyboard
      });
      ...
      modalDiv.addEventListener('hidden.bs.modal', () => {
        if (this._options.onModalClose) {
          this._options.onModalClose(this._state);
        }
        this._destroyModal();
      });
      ...
      console.error('[DXInterpreter] Bootstrap 5 Modal not found. Please include Bootstrap 5 JS.');
    }
  ...
  /**
   * Get the render target based on render mode
   */
  _getRenderTarget() {
    if (this._renderMode === 'modal') {
      if (!this._modalBody) {
 ...
 _renderStep() {
	...
	// Get appropriate render target (modal or embedded)
	const renderTarget = this._getRenderTarget();
	...
	const btnRow = _el('div', 'd-flex gap-2 justify-content-between align-items-center mt-4 pt-3 border-top');
	...
	form.appendChild(btnRow);  // ✅ INSIDE the form now

	/* Assemble */
	body.appendChild(form);
	card.appendChild(header);
	card.appendChild(body);
	// No separate footer element needed
	renderTarget.appendChild(card);

	/* Initial visibility pass */
	VisibilityEngine.applyAll(renderTarget, this._state);
  ...
  _handleSubmit(step, form, submitBtn) {
		...
		// 3. Resolve post endpoint safely
		const postUrl = this._resolvePostEndpoint();
		if (!postUrl) {
			this._showFormAlert(form, 'No submission endpoint configured.', 'danger');
			return;
		}
		...
		.then(response => {
			...
			if (response.status === 'success') {
				...
				if (response.next_step) {
					// Try exact match on step_id
					let nextIdx = (this._flow.steps ?? [])
						.findIndex(s => s.step_id === response.next_step);

					// Fallback: numeric index
					if (nextIdx === -1) {
						const asNum = Number(response.next_step);
						if (!isNaN(asNum) && asNum >= 0 && asNum < (this._flow.steps ?? []).length) {
							nextIdx = asNum;
						}
					}

					// Fallback: sequential advance
					if (nextIdx === -1 && this._stepIndex + 1 < (this._flow.steps ?? []).length) {
						console.warn(`[DXInterpreter] next_step "${response.next_step}" not found; advancing sequentially.`);
						nextIdx = this._stepIndex + 1;
					}
					...
					if (nextIdx > -1) {
						...
						if (typeof this._options.onStepComplete === 'function') {
							this._options.onStepComplete({
								step: step.step_id,
				...
				if (this._renderMode === 'modal' && this._options.modalOptions?.closeOnComplete !== false) {
	...
	_resolvePostEndpoint() {
		const pe = this._flow?.post_endpoint;
		if (!pe) return this._endpoint;  // fallback to GET endpoint
		if (pe.startsWith('http') || pe.startsWith('/')) return pe;
		// Resolve relative to the GET endpoint's directory
		const base = this._endpoint.substring(0, this._endpoint.lastIndexOf('/') + 1);
		return base + pe;
	}
  ...
  _showError(message) {
    this._target.innerHTML = '';
    const wrap = _el('div', 'alert alert-danger m-3');
  ...
  _attachVisibilityListeners(form) {
    const update = e => {
      ...
      this._applyVisibilityIfChanged();
    };
  ...
  _applyVisibilityIfChanged() {
    const sig = JSON.stringify(this._state ?? {});
